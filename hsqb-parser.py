...
def process_url(url):
    ...
    if len([el.get_text() for el in soup.find_all(['span', 'h5']) if re.search('YellowFruit', el.get_text())]) == 0:
        ...
        def process_game(game):
            ...
            team2 = re.match('^.*(?=\s[-\d]+$)', split_score[1])[0]

            line_split = [el for el in re.split(
                '\\n', game['line']) if re.search(':', el)]
            player_lines = {re.split(':\s', el)[0]: re.split(':\s', el)[
            ...
            bonuses = [re.split(',\s', el[9:])
                       for el in line_split if re.match('^Bonuses:', el)][0]
            bonuses = pd.DataFrame(
                [re.split('\s(?=[-\d])', team) for team in bonuses],
                columns=['team', 'bonuses_heard', 'bonus_pts', 'PPB']
            )
        ...
        for k, v in cleaned_rounds.items():
            player_stats = pd.concat([process_game(game)[0] for game in v])
            team_stats = pd.concat([process_game(game)[1] for game in v])
        ...
        if 'powers' in all_player_stats.columns:
            all_player_stats = all_player_stats[[
                'player', 'team', 'round', 'powers', 'gets', 'negs', 'pts']]
        else:
            all_player_stats = all_player_stats[[
                'player', 'team', 'round', 'gets', 'negs', 'pts']]
        all_team_stats = pd.concat(all_team_stats)
        if 'powers' in all_team_stats.columns:
            all_team_stats = all_team_stats[[
                'team', 'round', 'opponent', 'powers', 'gets', 'negs', 'bonuses_heard', 'bonus_pts', 'total_pts']]
        else:
            all_team_stats = all_team_stats[[
                'team', 'round', 'opponent', 'gets', 'negs', 'bonuses_heard', 'bonus_pts', 'total_pts']]

        return all_player_stats, all_team_stats

    else:
        def process_game_yf(game):
        ...
        for el in soup.find_all(["h3", "h2"]):
            if re.search('^Round\s\d+$', el.get_text()):
                ...
                cur_round.append(el.get_text().replace(
                    '\n\xa0\xa0\xa0\xa0\n', ''))
        rounds[cur_round_name] = cur_round
        ...
        all_team_stats = []
        for k, v in cleaned_rounds.items():
            player_stats = pd.concat([process_game_yf(game)[0] for game in v])
            team_stats = pd.concat([process_game_yf(game)[1] for game in v])
